...
use temporal_sdk_core_protos::{
    coresdk::{
        ...
        AsPayloadExt, FromFailureExt, FromPayloadExt,
    },
...
pub fn into_activity_0_args<F, R, O>(
    f: F,
) -> impl Fn(ActContext) -> BoxFuture<'static, Result<ActExitValue<O>, anyhow::Error>> + Send + Sync
where
    F: AsyncFn1<ActContext, Output = Result<R, anyhow::Error>> + Send + Sync + 'static,
    R: Into<ActExitValue<O>>,
    O: AsPayloadExt + Debug,
{
    move |ctx: ActContext| (f)(ctx).map(|r| r.map(|r| r.into())).boxed()
}
...
pub async fn execute_activity_0_args<F, R>(
    ctx: &WfContext,
    ...
    R: FromPayloadExt + Debug,
{
    ...
    match activity_resolution.status {
        Some(activity_resolution::Status::Completed(activity_result::Success { result })) => {
            Ok(R::from_payload(None, &result.unwrap()).unwrap())
        }
...
/// serialization into Temporal [Payload](temporal_sdk_core_protos::temporal::api::common::v1::Payload).
/// Use [into_activity_0_args] to register the activity with the worker.
pub async fn execute_activity_0_args_exit_value<F, R>(
    ctx: &WfContext,
    options: ActivityOptions,
    _f: F,
) -> Result<R, anyhow::Error>
...
where
    ...
    O: AsPayloadExt + Debug,
{
    move |_ctx: ActContext| (f)().map(|r| r.map(|r| r.into())).boxed()
}
...
pub async fn execute_activity_0_args_without_ctx<F, R>(
    ctx: &WfContext,
    options: ActivityOptions,
    _f: F,
) -> Result<R, anyhow::Error>
...
/// or any context dependent actions.
/// Use [into_activity_0_args_without_ctx] to register the activity with the worker.
pub async fn execute_activity_0_args_without_ctx_exit_value<F, R>(
    ctx: &WfContext,
    options: ActivityOptions,
    _f: F,
) -> Result<R, anyhow::Error>
...
where
    ...
    O: AsPayloadExt + Debug,
{
    move |ctx: ActContext| match A::from_payload(None, &ctx.get_args()[0]) {
...
pub async fn execute_activity_1_args<A, F, R>(
    ctx: &WfContext,
    options: ActivityOptions,
    _f: F,
    a: A,
...
where
    F: AsyncFn2<ActContext, A, Output = Result<R, anyhow::Error>> + Send + Sync + 'static,
    A: AsPayloadExt + Debug,
    R: FromPayloadExt + Debug,
{
    let a = A::as_payload(&a, None).expect("serializes fine");
    let activity_type = if options.activity_type.is_empty() {
        std::any::type_name::<F>().to_string()
    } else {
        options.activity_type
...
/// serialization into Temporal [Payload](temporal_sdk_core_protos::temporal::api::common::v1::Payload).
/// Use [into_activity_1_args] to register the activity with the worker.
pub async fn execute_activity_1_args_exit_value<A, F, R>(
    ctx: &WfContext,
    options: ActivityOptions,
    _f: F,
    a: A,
...
pub async fn execute_activity_1_args_with_errors<A, F, R, E>(
    ctx: &WfContext,
    options: ActivityOptions,
    _f: F,
    a: A,
...
where
    A: FromPayloadExt + Send,
    B: FromPayloadExt + Send,
    F: AsyncFn3<ActContext, A, B, Output = Result<R, anyhow::Error>> + Send + Sync + 'static,
    R: Into<ActExitValue<O>>,
    O: AsPayloadExt + Debug,
{
    move |ctx: ActContext| match A::from_payload(None, &ctx.get_args()[0]) {
        Ok(a) => match B::from_payload(None, &ctx.get_args()[1]) {
            Ok(b) => (f)(ctx, a, b).map(|r| r.map(|r| r.into())).boxed(),
...
pub async fn execute_activity_2_args<A, B, F, R>(
    ctx: &WfContext,
    options: ActivityOptions,
    _f: F,
    a: A,
...
where
    F: AsyncFn3<ActContext, A, B, Output = Result<R, anyhow::Error>> + Send + Sync + 'static,
    A: AsPayloadExt + Debug,
    B: AsPayloadExt + Debug,
    R: FromPayloadExt,
{
    let a = A::as_payload(&a, None).expect("serializes fine");
    let b = B::as_payload(&b, None).expect("serializes fine");
    let activity_type = if options.activity_type.is_empty() {
        std::any::type_name::<F>().to_string()
    } else {
        options.activity_type
    ...
    Ok(R::from_payload(None, &output).unwrap())
}
...
where
    ...
    O: AsPayloadExt + Debug,
{
    move |ctx: WfContext| (f)(ctx).map(|r| r.map(|r| r.into())).boxed()
}
...
pub async fn execute_child_workflow_0_args<F, R>(
    ctx: &WfContext,
...
/// serialization into Temporal [Payload](temporal_sdk_core_protos::temporal::api::common::v1::Payload).
/// Use [into_workflow_0_args] to register the workflow with the worker.
pub async fn execute_child_workflow_0_args_exit_value<F, R>(
    ctx: &WfContext,
    options: ChildWorkflowOptions,
    _f: F,
) -> Result<R, anyhow::Error>
...
where
    ...
    O: AsPayloadExt + Debug,
{
    move |ctx: WfContext| match A::from_payload(None, &ctx.get_args()[0]) {
        Ok(a) => (f)(ctx, a).map(|r| r.map(|r| r.into())).boxed(),
        Err(e) => async move { Err(e.into()) }.boxed(),
    }
}
...
pub async fn execute_child_workflow_1_args<A, F, R>(
    ctx: &WfContext,
    options: ChildWorkflowOptions,
    _f: F,
    a: A,
...
pub async fn execute_child_workflow_1_args_exit_value<A, F, R>(
    ctx: &WfContext,
    options: ChildWorkflowOptions,
    _f: F,
    a: A,
...
/// serialization into Temporal [Payload](temporal_sdk_core_protos::temporal::api::common::v1::Payload).
/// Use [into_workflow_0_args] to register the workflow with the worker.
pub async fn execute_child_workflow_1_args_with_errors<A, F, R, E>(
    ctx: &WfContext,
    options: ChildWorkflowOptions,
    _f: F,
    a: A,
...
pub async fn execute_child_workflow_2_args<A, B, F, R>(
    ctx: &WfContext,
    options: ChildWorkflowOptions,
    _f: F,
    a: A,
...
/// serialization into Temporal [Payload](temporal_sdk_core_protos::temporal::api::common::v1::Payload).
/// Use [into_workflow_2_args] to register the workflow with the worker
pub async fn execute_child_workflow_2_args_exit_value<A, B, F, R>(
    ctx: &WfContext,
    options: ChildWorkflowOptions,
    _f: F,
    a: A,
    ...
    R: FromPayloadExt + Debug,
{
    ...
    match started.result().await.status {
        Some(child_workflow_result::Status::Completed(child_workflow::Success { result })) => Ok(
            WfExitValue::Normal(R::from_payload(None, &result.unwrap()).unwrap()),
        ),
        _ => Err(anyhow!("Unexpected child WF status")),
    }
}
