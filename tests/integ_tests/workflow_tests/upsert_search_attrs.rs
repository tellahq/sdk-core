...
use temporal_sdk_core_protos::coresdk::{AsPayloadExt, FromPayloadExt};
...
async fn search_attr_updater(ctx: WfContext) -> WorkflowResult<()> {
    ctx.upsert_search_attributes([
        (TXT_ATTR.to_string(), "goodbye".as_payload(None).unwrap()),
        (INT_ATTR.to_string(), 98.as_payload(None).unwrap()),
    ]);
    Ok(().into())
}
...
async fn sends_upsert() {
        ...
        .submit_wf(
            ...
            WorkflowOptions {
                search_attributes: Some(HashMap::from([
                    (TXT_ATTR.to_string(), "hello".as_payload(None).unwrap()),
                    (INT_ATTR.to_string(), 1.as_payload(None).unwrap()),
                ])),
                ..Default::default()
            },
        )
    ...
    assert_eq!(
        "goodbye",
        String::from_payload(None, txt_attr_payload).unwrap()
    );
    assert_eq!(98, usize::from_payload(None, int_attr_payload).unwrap());
}
