...
async fn memo_upserter(ctx: WfContext) -> WorkflowResult<()> {
    ctx.upsert_memo([
        (FIELD_A.to_string(), "enchi".as_payload(None).unwrap()),
        (FIELD_B.to_string(), 9001.as_payload(None).unwrap()),
    ]);
    Ok(().into())
}
...
async fn sends_modify_wf_props() {
    ...
    assert_eq!("enchi", String::from_payload(None, catname).unwrap());
    assert_eq!(9001, usize::from_payload(None, cuteness).unwrap());
}
