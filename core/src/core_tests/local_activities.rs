...
use temporal_sdk::{
    ActContext, ActivityCancelledError, ActivityFunction, LocalActivityOptions, WfContext,
...
async fn local_act_two_wfts_before_marker(#[case] replay: bool, #[case] cached: bool) {
    ...
    worker.register_activity(DEFAULT_ACTIVITY_TYPE, ActivityFunction::new(echo));
    worker
        .submit_wf(
            wf_id.to_owned(),
            DEFAULT_WORKFLOW_TYPE.to_owned(),
...
async fn local_act_many_concurrent() {
    ...
    worker.register_activity("echo", ActivityFunction::new(echo));
    worker
        .submit_wf(
            wf_id.to_owned(),
            DEFAULT_WORKFLOW_TYPE.to_owned(),
...
async fn local_act_heartbeat(#[case] shutdown_middle: bool) {
    ...
    worker.register_activity(
        "echo",
        ActivityFunction::new(move |_ctx: ActContext, str: String| async move {
            if shutdown_middle {
...
async fn local_act_fail_and_retry(#[case] eventually_pass: bool) {
    ...
    worker.register_activity(
        "echo",
        ActivityFunction::new(move |_ctx: ActContext, _: String| async move {
...
async fn local_act_null_result() {
    ...
    worker.register_activity(
        "nullres",
        ActivityFunction::new(|_ctx: ActContext, _: String| async { Ok(()) }),
    );
    worker
        .submit_wf(
            wf_id.to_owned(),
...
async fn test_schedule_to_start_timeout() {
    ...
    worker.register_activity(
        "echo",
        ActivityFunction::new(move |_ctx: ActContext, _: String| async move { Ok(()) }),
    );
    worker
        .submit_wf(
            wf_id.to_owned(),
...
async fn resolved_las_not_recorded_if_wft_fails_many_times() {
    ...
    worker.register_activity(
        "echo",
        ActivityFunction::new(move |_: ActContext, _: String| async move { Ok(()) }),
    );
    worker
        .submit_wf(
            wf_id.to_owned(),
